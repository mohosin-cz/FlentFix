...
// Labour keywords — items matching these are shown in Labour RC tab
...
function TradePill({ label, active, onClick }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        border: active
          ? `1px solid ${meta?.border || 'rgba(200,150,62,0.4)'}`
          : '1px solid var(--border, #2e3040)',
        background: active
          ? (meta?.bg || 'rgba(200,150,62,0.1)')
          : 'var(--bg-input, #252731)',
        color: active
          ? (meta?.color || 'var(--accent, #c8963e)')
...
export default function InternalRateCard() {
  ...
  const [editing, setEditing]   = useState({}) // fxin → { manual_override, unit }
  ...
  const tabFiltered = rows.filter(r =>
    tab === 'Labour RC' ? isLabourItem(r) : !isLabourItem(r)
  )

  // Trade pill filter
  const tradeFiltered = tradePill === 'All'
    ? tabFiltered
    : tabFiltered.filter(r => r.trade?.toLowerCase() === tradePill.toLowerCase())

  // Search filter
  const displayed = filter
    ? tradeFiltered.filter(r =>
    ...
    <div style={{ minHeight: '100svh', background: 'var(--bg, #16171f)', fontFamily: 'var(--font-sans, Poppins, sans-serif)', color: 'var(--text, #e8e8f0)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={s.tabBar}>
        {['Labour RC', 'Material RC'].map(t => (
          <button
            key={t}
            onClick={() => { setTab(t); setTradePill('All'); setFilter('') }}
            style={{ ...s.tabBtn, color: tab === t ? 'var(--accent, #c8963e)' : 'var(--text-muted, #6b6d82)', borderBottom: tab === t ? '2px solid var(--accent, #c8963e)' : '2px solid transparent' }}
          >{t}</button>
        ))}
      </div>
      ...
      <div style={s.pillRow}>
        {TRADES.map(t => (
          <TradePill key={t} label={t} active={tradePill === t} onClick={() => setTradePill(t)} />
        ))}
      </div>

      {/* ── Info banner ── */}
      <div style={{ maxWidth: 800, margin: '12px auto 0', width: '100%', padding: '0 16px' }}>
      ...
      {/* ── Table ── */}
      <div style={{ flex: 1, maxWidth: 800, margin: '14px auto 40px', width: '100%', padding: '0 16px' }}>
        {displayed.length === 0 ? (
            ...
            {displayed.map((row, i) => {
              ...
              return (
                <div key={row.fxin} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 14px', borderTop: '1px solid var(--border, #2e3040)', minHeight: 44, background: i % 2 !== 0 ? 'rgba(255,255,255,0.018)' : 'transparent' }}>
                  ...
                  {/* Avg cost */}
                  <span style={{ width: 90, flexShrink: 0, fontSize: 12, color: 'var(--text-muted, #6b6d82)', textAlign: 'right', fontFamily: 'var(--font-mono, monospace)' }}>
                    ₹{(row.avg_cost || 0).toLocaleString('en-IN')}
                  </span>

                  {/* Last price */}
                  <span style={{ width: 90, flexShrink: 0, fontSize: 12, color: 'var(--text-muted, #6b6d82)', textAlign: 'right', fontFamily: 'var(--font-mono, monospace)' }}>
                    ₹{(row.last_price || 0).toLocaleString('en-IN')}
                  </span>

                  {/* Effective price — editable when admin */}
                  {isEditing ? (
                    <div style={{ width: 110, flexShrink: 0 }}>
                    ...
                    <span style={{ width: 110, flexShrink: 0, fontSize: 13, fontWeight: 700, color: overridden ? 'var(--accent, #c8963e)' : 'var(--text, #e8e8f0)', textAlign: 'right', fontFamily: 'var(--font-mono, monospace)' }}>
                      ₹{(effective || 0).toLocaleString('en-IN')}
                    </span>
                  )}
...
const s = {
  ...
  tabBar: { display: 'flex', background: 'var(--bg-panel, #1e2028)', borderBottom: '1px solid var(--border, #2e3040)', padding: '0 20px', gap: 2 },
  tabBtn: { padding: '11px 20px', fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-mono, monospace)', cursor: 'pointer', background: 'none', border: 'none', transition: 'color 0.15s, border-color 0.15s' },
  pillRow: { display: 'flex', gap: 6, padding: '12px 16px', maxWidth: 800, margin: '0 auto', width: '100%', flexWrap: 'wrap' },
