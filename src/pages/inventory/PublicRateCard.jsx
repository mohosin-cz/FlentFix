...
function Tabs({ active, onChange }) {
  return (
    <div style={{ display: 'flex', gap: 4, padding: '14px 16px 0', maxWidth: 700, margin: '0 auto', width: '100%' }}>
      {['Labour RC', 'Material RC'].map(t => (
        <button key={t} onClick={() => onChange(t)} style={{ padding: '7px 18px', fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-mono, monospace)', cursor: 'pointer', background: 'none', border: 'none', borderBottom: active === t ? '2px solid var(--accent, #c8963e)' : '2px solid transparent', color: active === t ? 'var(--accent, #c8963e)' : 'var(--text-muted, #6b6d82)', transition: 'color 0.15s' }}>
          {t}
        </button>
      ))}
    </div>
...
export default function PublicRateCard() {
  ...
  const tabRows = rows.filter(r =>
    tab === 'Labour RC'
      ? (r.labour_cost > 0 || (!r.material_cost || r.material_cost === 0))
      : (r.material_cost > 0)
  )

  const grouped = tabRows.reduce((acc, row) => {
  ...
  async function saveRow(row) {
    ...
    await supabase.from('rate_card').update({ material_cost: parseFloat(e.material_cost) || 0, labour_cost: parseFloat(e.labour_cost) || 0, unit: e.unit }).eq('id', row.id)
  ...
  async function addNewItem(area) {
    ...
    const { data, error } = await supabase.from('rate_card').insert({ area, item_name: newRow.item_name, material_cost: parseFloat(newRow.material_cost) || 0, labour_cost: parseFloat(newRow.labour_cost) || 0, unit: newRow.unit || '' }).select().single()
    if (!error && data) { setRows(prev => [...prev, data]); setNewRow({ item_name: '', area: '', material_cost: '', labour_cost: '', unit: '' }); setAddingTo(null) }
  }

  if (loading) return <div style={{ minHeight: '100svh', background: 'var(--bg, #16171f)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted, #6b6d82)', fontFamily: 'var(--font-mono, monospace)', fontSize: 13 }}>loading…</div>
  ...
  return (
    <div style={{ minHeight: '100svh', background: 'var(--bg, #16171f)', fontFamily: 'var(--font-sans, Poppins, sans-serif)', color: 'var(--text, #e8e8f0)' }}>
      ...
      <Tabs active={tab} onChange={t => { setTab(t); setFilter('') }} />
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '16px 16px 60px' }}>
        ...
        <div style={{ position: 'relative', marginBottom: 20 }}>
          ...
          <input value={filter} onChange={e => setFilter(e.target.value)} placeholder="Search items or trade…"
        ...
        {filteredGroups.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-muted, #6b6d82)', fontFamily: 'var(--font-mono, monospace)', fontSize: 12 }}>No items in this category.</div>
        )}
                  ...
                  <div key={row.id} style={{ ...s.tableRow, background: ri % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)' }}>
                    {isEditing ? (
                      <>
                        ...
                        <div style={{ flex: 1 }}><Inp type="number" value={e.material_cost} onChange={v => setEditing(p => ({ ...p, [row.id]: { ...p[row.id], material_cost: v } }))} placeholder="0" /></div>
                        <div style={{ flex: 1 }}><Inp type="number" value={e.labour_cost}   onChange={v => setEditing(p => ({ ...p, [row.id]: { ...p[row.id], labour_cost: v } }))}   placeholder="0" /></div>
                        <span style={{ flex: 1, fontSize: 12, color: 'var(--text-muted, #6b6d82)', textAlign: 'right' }}>—</span>
                        <div style={{ flex: 1 }}><Inp value={e.unit} onChange={v => setEditing(p => ({ ...p, [row.id]: { ...p[row.id], unit: v } }))} placeholder="nos" /></div>
