...
function TradePill({ trade, active, onClick, small }) {
...
function EditInput({ value, onChange, placeholder, type = 'text' }) {
  return (
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      style={{ width: '100%', padding: '5px 8px', fontSize: 12, color: 'var(--text, #e8e8f0)', background: 'var(--bg, #16171f)', border: '1px solid var(--accent, #c8963e)', borderRadius: 5, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
    />
  )
}
...
export default function PublicRateCard() {
  ...
  function startEdit(row) {
    setEditing(prev => ({ ...prev, [row.id]: { material_cost: String(row.material_cost ?? ''), labour_cost: String(row.labour_cost ?? ''), unit: row.unit || '', area: row.area || '' } }))
  }
  ...
  async function saveRow(row) {
    ...
    const patch = { material_cost: parseFloat(e.material_cost) || 0, labour_cost: parseFloat(e.labour_cost) || 0, unit: e.unit, area: e.area }
  ...
  async function addItem(trade) {
    ...
    const { data, error } = await supabase.from('rate_card').insert({
      trade, area: newRow.area || '', item_name: newRow.item_name,
      material_cost: parseFloat(newRow.material_cost) || 0,
      labour_cost: parseFloat(newRow.labour_cost) || 0, unit: newRow.unit || '',
    }).select().single()
    if (!error && data) {
  ...
  async function confirmImport() {
    ...
    if (!error && data) { setRows(prev => [...prev, ...data]); setPreview(null) }
    else alert('Import failed: ' + (error?.message || 'unknown error'))
    setImporting(false)
  }

  if (loading) return (
    <div style={{ minHeight: '100svh', background: 'var(--bg, #16171f)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted, #6b6d82)', fontFamily: 'var(--font-mono, monospace)', fontSize: 13 }}>loading…</div>
  )
  ...
  const containerStyle = {
    ...
    margin: '0 auto',
    padding: isMobile ? '14px 16px 80px' : '14px 24px 80px',
  ...
  return (
    <div style={{ minHeight: '100svh', background: 'var(--bg, #16171f)', fontFamily: 'var(--font-sans, Poppins, sans-serif)', color: 'var(--text, #e8e8f0)' }}>
      ...
      <div style={containerStyle}>
        ...
        {/* Trade pills — wrap on mobile with smaller padding */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
          {TRADES.map(t => (
            <TradePill key={t} trade={t} active={tradePill === t} onClick={() => setTradePill(t)} small={isMobile} />
          ))}
        </div>
        ...
        {isAdmin && (
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, marginBottom: 14 }}>
            <button onClick={downloadTemplate}
              style={{ padding: '7px 14px', fontSize: 11, fontWeight: 600, cursor: 'pointer', background: 'var(--bg-panel, #1e2028)', border: '1px solid var(--border, #2e3040)', borderRadius: 6, color: 'var(--text-muted, #6b6d82)', fontFamily: 'var(--font-mono, monospace)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
            ...
            <button onClick={() => fileRef.current?.click()}
              style={{ padding: '7px 14px', fontSize: 11, fontWeight: 600, cursor: 'pointer', background: 'rgba(200,150,62,0.08)', border: '1px solid rgba(200,150,62,0.3)', borderRadius: 6, color: 'var(--accent, #c8963e)', fontFamily: 'var(--font-mono, monospace)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
        ...
        <div style={{ padding: '8px 12px', background: 'rgba(200,150,62,0.06)', border: '1px solid rgba(200,150,62,0.15)', borderRadius: 7, marginBottom: 14, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--text-dim, #9394a8)', fontFamily: 'var(--font-mono, monospace)', lineHeight: 1.5 }}>
            {tab === 'Labour RC' ? 'Labour RC — items with labour cost and no material cost.' : 'Material RC — items with material cost.'}
          </span>
        </div>
        ...
        {preview && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            <div style={{ background: 'var(--bg-panel, #1e2028)', border: '1px solid var(--border, #2e3040)', borderRadius: 12, width: '100%', maxWidth: 700, maxHeight: '85svh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  ...
                  preview.map((row, i) => (
                    <div key={i} style={{ padding: '11px 16px', borderTop: i > 0 ? '1px solid var(--border, #2e3040)' : 'none', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text, #e8e8f0)', fontFamily: 'var(--font-mono, monospace)' }}>₹{(row.material_cost + row.labour_cost).toLocaleString('en-IN')}</span>
                      </div>
                      <div style={{ fontSize: 10, color: TRADE_META[row.trade?.toLowerCase()]?.color || '#9394a8', fontFamily: 'var(--font-mono, monospace)', marginTop: 3 }}>{row.trade} {row.area ? `· ${row.area}` : ''}</div>
                      <div style={{ fontSize: 10, color: 'var(--text-dim, #9394a8)', fontFamily: 'var(--font-mono, monospace)', marginTop: 2 }}>Material: ₹{row.material_cost} · Labour: ₹{row.labour_cost}</div>
                    </div>
                  ))
                ) : (
                  <>
              ...
              <div style={{ padding: '12px 18px', borderTop: '1px solid var(--border, #2e3040)', display: 'flex', gap: 8, flexDirection: isMobile ? 'column' : 'row' }}>
                <button onClick={confirmImport} disabled={importing}
                  style={{ padding: '9px 20px', background: 'var(--accent, #c8963e)', color: '#fff', border: 'none', borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: importing ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-mono, monospace)', opacity: importing ? 0.6 : 1 }}>
                  {importing ? 'Importing…' : `Confirm Import (${preview.length} rows)`}
                </button>
                <button onClick={() => setPreview(null)}
                  style={{ padding: '9px 16px', background: 'var(--bg-input, #252731)', color: 'var(--text-muted, #6b6d82)', border: '1px solid var(--border, #2e3040)', borderRadius: 7, fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-mono, monospace)' }}>
                  Cancel
                </button>
              </div>
            </div>
              ...
              <div style={{ background: 'var(--bg-panel, #1e2028)', borderRadius: 10, border: '1px solid var(--border, #2e3040)', overflow: 'hidden' }}>
                ...
                {items.map((row, ri) => {
                  ...
                  if (isMobile) {
                    // Mobile card layout
                    return (
                      <div key={row.id} style={{ padding: '13px 14px', borderTop: ri > 0 ? '1px solid var(--border, #2e3040)' : 'none', background: ri % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)' }}>
                        {isEdit ? (
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text, #e8e8f0)', marginBottom: 10 }}>{row.item_name}</div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                              <div><span style={s.label}>Area</span><EditInput value={e.area} onChange={v => setEditing(p => ({ ...p, [row.id]: { ...p[row.id], area: v } }))} placeholder="area" /></div>
                              <div><span style={s.label}>Unit</span><EditInput value={e.unit} onChange={v => setEditing(p => ({ ...p, [row.id]: { ...p[row.id], unit: v } }))} placeholder="nos" /></div>
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                              <div><span style={s.label}>Material ₹</span><EditInput type="number" value={e.material_cost} onChange={v => setEditing(p => ({ ...p, [row.id]: { ...p[row.id], material_cost: v } }))} placeholder="0" /></div>
                              <div><span style={s.label}>Labour ₹</span><EditInput type="number" value={e.labour_cost} onChange={v => setEditing(p => ({ ...p, [row.id]: { ...p[row.id], labour_cost: v } }))} placeholder="0" /></div>
                            </div>
                          ...
                          <div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text, #e8e8f0)', flex: 1, paddingRight: 8 }}>{row.item_name}</span>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text, #e8e8f0)', fontFamily: 'var(--font-mono, monospace)' }}>₹{total.toLocaleString('en-IN')}</span>
                                {isAdmin && (
                                  <button onClick={() => startEdit(row)} style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: '1px solid var(--border, #2e3040)', borderRadius: 5, color: 'var(--text-muted, #6b6d82)', cursor: 'pointer', flexShrink: 0 }}>
                                    <PencilIcon />
                                  </button>
                                )}
                              </div>
                            </div>
                            <div style={{ fontSize: 11, color: 'var(--text-muted, #6b6d82)', marginBottom: 3 }}>
                              {row.area || '—'} · {row.unit ? `per ${row.unit}` : 'per unit'}
                            </div>
                            <div style={{ fontSize: 11, color: 'var(--text-dim, #9394a8)', fontFamily: 'var(--font-mono, monospace)' }}>
                              Material: ₹{(row.material_cost || 0).toLocaleString('en-IN')} · Labour: ₹{(row.labour_cost || 0).toLocaleString('en-IN')}
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      {isEdit ? (
                        <>
                          ...
                          <div style={{ display: 'flex', gap: 3, justifyContent: 'flex-end' }}>
                            <button onClick={() => saveRow(row)} disabled={saving[row.id]} style={{ width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--green, #3dba7a)', border: 'none', borderRadius: 4, color: '#fff', fontSize: 11, fontWeight: 700, cursor: saving[row.id] ? 'not-allowed' : 'pointer' }}>{saving[row.id] ? '…' : '✓'}</button>
                            <button onClick={() => cancelEdit(row.id)} style={{ width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-input, #252731)', border: '1px solid var(--border, #2e3040)', borderRadius: 4, color: 'var(--text-muted, #6b6d82)', fontSize: 11, cursor: 'pointer' }}>✕</button>
                          </div>
                        </>
                      ) : (
                        <>
                          ...
                          {isAdmin && (
                            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                              <button onClick={() => startEdit(row)} style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: '1px solid var(--border, #2e3040)', borderRadius: 5, color: 'var(--text-muted, #6b6d82)', cursor: 'pointer' }} title="Edit row">
                                <PencilIcon />
                              </button>
                            </div>
                          )}
                ...
                {isAdmin && addingTo === trade && (
                  <div style={{ padding: '14px', background: 'rgba(200,150,62,0.04)', borderTop: '1px solid var(--border, #2e3040)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '2fr 1fr 1fr 1fr 1fr', gap: 8, marginBottom: 10 }}>
                      <div><span style={s.label}>Item Name</span><EditInput value={newRow.item_name} onChange={v => setNewRow(p => ({ ...p, item_name: v }))} placeholder="item name" /></div>
                      <div><span style={s.label}>Area</span><EditInput value={newRow.area} onChange={v => setNewRow(p => ({ ...p, area: v }))} placeholder="e.g. Bathroom" /></div>
                      <div><span style={s.label}>Material ₹</span><EditInput type="number" value={newRow.material_cost} onChange={v => setNewRow(p => ({ ...p, material_cost: v }))} placeholder="0" /></div>
                      <div><span style={s.label}>Labour ₹</span><EditInput type="number" value={newRow.labour_cost} onChange={v => setNewRow(p => ({ ...p, labour_cost: v }))} placeholder="0" /></div>
                      <div><span style={s.label}>Unit</span><EditInput value={newRow.unit} onChange={v => setNewRow(p => ({ ...p, unit: v }))} placeholder="nos" /></div>
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => addItem(trade)} style={{ flex: isMobile ? 1 : undefined, padding: '7px 18px', background: 'var(--accent, #c8963e)', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-mono, monospace)' }}>Save Item</button>
                      <button onClick={() => setAddingTo(null)} style={{ flex: isMobile ? 1 : undefined, padding: '7px 14px', background: 'var(--bg-input, #252731)', color: 'var(--text-muted, #6b6d82)', border: '1px solid var(--border, #2e3040)', borderRadius: 6, fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-mono, monospace)' }}>Cancel</button>
                    </div>
                  </div>
                )}
...
const s = {
  ...
  label: { fontSize: 10, fontWeight: 600, color: 'var(--text-dim, #9394a8)', textTransform: 'uppercase', letterSpacing: '0.07em', fontFamily: 'var(--font-mono, monospace)', display: 'block', marginBottom: 4 },
}
