...
// Cross-property utilities — searchable, filterable, sorted by soonest recharge.
export default function PropertyUtilitiesOverview() {
  ...
  const dueSoon = useMemo(() => rows.filter(r => r.bucket === 'due').length, [rows])
  const monthlySpend = useMemo(() => rows.reduce((a, r) => a + monthlyCost(r), 0), [rows])
          ...
          {loading ? <LogoSpinner /> : error ? (
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                <Stat label="Due this week" value={dueSoon} color={dueSoon ? 'var(--red, #e05c6a)' : 'var(--text-dim, #9394a8)'} />
                <Stat label="Active" value={rows.length} color="var(--text, #e8e8f0)" />
                <Stat label="Monthly spend" value={money(monthlySpend)} color="var(--accent, #c8963e)" small />
              </div>
              ...
              {presentTypes.length > 1 && (
                <div style={{ display: 'flex', gap: 7, overflowX: 'auto', paddingBottom: 8, marginBottom: 2, WebkitOverflowScrolling: 'touch' }}>
...
function Stat({ label, value, color, small }) {
  return (
    <div style={{ flex: 1, minWidth: 0, background: 'var(--bg-panel, #1e2028)', border: '1px solid var(--border, #2e3040)', borderRadius: 12, padding: '12px 13px' }}>
      <div style={{ fontSize: small ? 17 : 22, fontWeight: 800, color, fontFamily: MONO, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{value}</div>
      <div style={{ fontSize: 10, color: 'var(--text-muted, #6b6d82)', fontFamily: MONO, textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 3 }}>{label}</div>
    </div>
  )
}
...
const s = {
  ...
  main: { flex: 1, padding: '20px 20px 48px', maxWidth: 640, width: '100%', margin: '0 auto' },
