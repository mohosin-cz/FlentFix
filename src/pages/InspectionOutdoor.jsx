...
function isDone(item) {
  ...
  return item.notAvailable || (item.issues || []).some(r => r.issueDescription)
}
...
function LabourRateDropdown({ rates, value, labourCost, onSelect }) {
  ...
  return (
    <Field label="Labour Rate" hint={value ? `₹${parseFloat(labourCost || 0).toLocaleString('en-IN')} auto-filled` : 'Select to auto-fill cost'}>
      <div style={{ position: 'relative' }}>
        <select value={value || ''} onChange={e => { const r = rates.find(x => x.id === e.target.value); onSelect(e.target.value, r ? String(r.cost_per_unit) : '') }} style={{ fontFamily: 'inherit', width: '100%', padding: '10px 36px 10px 14px', fontSize: 13, color: value ? 'var(--text, #e8e8f0)' : 'var(--text-muted, #6b6d82)', border: '1px solid var(--border, #2e3040)', borderRadius: 6, background: 'var(--bg-input, #252731)', outline: 'none', appearance: 'none', cursor: 'pointer' }}>
      ...
      SECTIONS[sectionKey].forEach(({ key, title, trade }) => {
        ...
        issueRows.forEach((row, ri) => {
          ...
          lineItemRows.push({
            ...
            item_score:          item.health != null ? item.health : (row.issueDescription === 'Functional' ? 10 : null),
